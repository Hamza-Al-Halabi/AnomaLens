...
if __name__ == "__main__":
    ...
    config = load_config()
    print("=" * 55)
    print("GRU — BATADAL quick test (seed=42)")
    ...
    model = GRUModel(config, X_tr_sc.shape[1])
    print(f"Parameters: {sum(p.numel() for p in model.parameters()):,}")

    result, trained = train_model(model, config,
                                   X_tr_sc, y_train.values,
                                   X_va_sc, y_val.values,
                                   seed=42, model_name="GRU")
    test_m = predict_model(trained, config, X_te_sc, y_test.values, model_name="GRU")
